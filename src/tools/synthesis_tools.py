...
def register_synthesis_tools(registry, provider: "DataProvider", memory: "MemoryRepository") -> None:
    registry.register(Tool(
        ...
        func=lambda date,**kw: _assess_regime(provider, date), category="synthesis"))
    registry.register(Tool(
        ...
        func=_build_candidates_impl, category="synthesis"))
    registry.register(Tool(
        ...
        func=_score_impl, category="synthesis"))
    registry.register(Tool(
        ...
        func=_narrative_impl, category="synthesis"))
    registry.register(Tool(
        name="format_report",
        ...
        func=_format_report_impl, category="synthesis"))
    registry.register(Tool(
...
async def _build_candidates_impl(stock_universe, theme_sectors):
    ...
    if any("market_cap" in s or "total_mv" in s for s in candidates[:1]):
        ...
        candidates = [s for s in candidates if (s.get("market_cap") or s.get("total_mv") or 0) >= 5_000_000_000]
...
async def _score_impl(candidates, market_context=None, watchlist_status=None):
    ...
    for i, c in enumerate(candidates[:10]):
        ...
        scored.append({"rank":i+1,"stock_code":code,"stock_name":name,"score":5.0,
            "score_breakdown":{"fundamental":5.0,"technical":5.0,"news_sentiment":5.0,"capital_flow":5.0,"watchlist_priority":5.0,"theme_alignment":5.0}})
...
async def _narrative_impl(scored_candidates, market_context=None, llm_client=None, **kw):
    ...
    ct = "\n".join(f"#{c['rank']} {c.get('stock_name','')} ({c.get('stock_code','')}): score {c.get('score',0):.1f}" for c in scored_candidates[:5])
    prompt = f"Write a concise morning market report narrative.\nMarket regime: {regime}\nTop candidates:\n{ct}\n\nInclude: market summary paragraph, one paragraph per candidate (thesis+evidence+risks), and one learning point."
    try:
...
def _md(data):
    ...
    if market:
        ...
        if market.get("risk_warning"): lines.append(f"> **Risk Warning:** {market['risk_warning']}")
        lines.append("")
    for c in data.get("candidates",[]):
        ...
        if c.get("thesis"): lines.append(f"**Thesis:** {c['thesis']}")
        if c.get("risks"): lines.append("**Risks:** " + "; ".join(c["risks"]))
        lines.append("")
    dd = data.get("deep_dive")
    if dd:
        lines.append("## Deep Dive")
        lines.append(f"### {dd.get('stock_name','')} ({dd.get('stock_code','')})")
        for k in ("business_summary","catalyst","valuation_context","risk_scenario","price_zone","invalidation"):
            if dd.get(k): lines.append(f"**{k.replace('_',' ').title()}:** {dd[k]}")
        lines.append("")
    ...
    if data.get("watchlist_alerts"):
        ...
        for a in data["watchlist_alerts"]: lines.append(f"- {a}")
        lines.append("")
    return "\n".join(lines)
