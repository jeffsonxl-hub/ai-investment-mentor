...
def register_memory_tools(registry, memory: "MemoryRepository") -> None:
    registry.register(Tool(name="get_watchlist",
        description="Fetch the user active watchlist.",
        parameters=[], func=lambda **kw: memory.get_watchlist(), category="memory"))
    registry.register(Tool(name="get_watchlist_entry",
        description="Fetch a single watchlist entry by stock code.",
        parameters=[ToolParameter("stock_code","string","6-digit stock code")],
        func=lambda stock_code,**kw: memory.get_watchlist_entry(stock_code), category="memory"))
    registry.register(Tool(name="get_recent_decisions",
        description="Fetch past recommendations.",
        parameters=[ToolParameter("days","integer","Days to look back",required=False,default=5)],
        func=lambda days=5,**kw: memory.get_recent_decisions(days), category="memory"))
    registry.register(Tool(name="get_rejected_stocks",
        ...
        func=lambda days=20,**kw: memory.get_rejected_stocks(days), category="memory"))
    registry.register(Tool(name="get_market_history",
        ...
        category="memory"))
    registry.register(Tool(name="save_market_snapshot",
        description="Save market analysis snapshot to memory.",
        parameters=[ToolParameter("snapshot","object","Market snapshot dict")],
        func=lambda snapshot,**kw: _write(memory.save_market_snapshot, snapshot), category="memory"))
    registry.register(Tool(name="save_decision",
        description="Save a recommendation decision to memory.",
        parameters=[ToolParameter("decision","object","Decision dict")],
        func=lambda decision,**kw: _write(memory.save_decision, decision), category="memory"))
    ...
    registry.register(Tool(name="update_watchlist_entry",
        ...
        func=lambda stock_code,priority,**kw: _write(memory.update_watchlist_priority, stock_code, priority),
        category="memory"))
