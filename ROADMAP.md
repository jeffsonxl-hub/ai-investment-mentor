...
### Phase 6  -- Tool Design
Designed. Tool is a distinct abstraction wrapping one or more Component calls behind an LLM-readable function-calling interface. Compatible with OpenAI function calling and MCP  -- identical JSON schema.

Key decisions:
- Tool != Component: Tools add natural-language descriptions the LLM reads to decide when to call them
- Tool != Agent: Tools are stateless, deterministic, executed on demand  -- never decide when to run
- Tools can orchestrate multiple Component calls (e.g., `assess_market_regime` calls 4 DataProvider methods)
- Per-Agent tool authorization enforced by ToolRegistry at schema-export time
- 34 Tools across 5 categories: Data (9), Analysis (6), Memory (8), LLM-Powered (4), Synthesis (7)
- 6 Analysis Tools (RSI, MACD, MA, Bollinger, Volume, Returns) demoted from "Technical Agent"  -- pure math, no LLM needed

Deliverables: [ADR-006](adr/ADR-006-tool-design.md), [design doc](docs/architecture/08-tool-design.md), [TASK-006](tasks/TASK-006-tool-implementation.md), [Tool spec](components/tool.md), [ToolRegistry spec](components/tool-registry.md). Implemented. 34 Tools across 8 source files. Tool, ToolParameter, ToolRegistry, factory. `_validate_params()` type checking, `dependencies` injection, dual schema export (OpenAI + MCP). 106 tests pass (34 new). `demo_phase6.py` runs end-to-end. Code reviewed: 6 findings fixed.
